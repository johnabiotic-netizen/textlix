...
function PromoCodeInput({ onApply }) {
  ...
  if (applied) return (
    <div className="flex items-center gap-2 text-sm text-green-600 bg-green-50 border border-green-200 rounded-xl px-4 py-3">
      ...
      <button onClick={() => { setApplied(null); setCode(''); onApply(null); }} className="ml-auto text-gray-400 hover:text-gray-600">✕</button>
    </div>
  );
  ...
  return (
    <div>
      {!open ? (
        <button onClick={() => setOpen(true)} className="text-sm text-brand-600 hover:underline">Have a promo code?</button>
      ) : (
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 border border-gray-300 rounded-xl px-3 py-2 text-sm font-mono uppercase focus:ring-2 focus:ring-brand-500 outline-none"
          />
